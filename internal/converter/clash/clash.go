...
func (r Clash) SubType() string {
	return "clash"
}

func (r Clash) Parse(content, sub string) ([]types.ProxyNode, error) {
...
func (r Clash) Convert2SingBoxOutbounds(node types.ProxyNode) map[string]any {
	return r.converterMap[node.Type].Convert2SingBox(node)
}
